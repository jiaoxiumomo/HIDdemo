...
int hid_write(hid_device *dev, const unsigned char *data, size_t length, bool NoReportID)
{
	DWORD bytes_written;
	BOOL res;
	...
	if (NoReportID)
	{
		if (length + 1 > dev->output_report_length) {
			return -2;
		}
		else { 
			/* Create a temporary buffer and copy the user's data into it. */
			buf = (unsigned char *) malloc(dev->output_report_length);			
			buf[0] = 0; /* 开头补充0作为没有报告ID的标志 */
			memcpy(buf + 1, data, length);
			if (length + 1 < dev->output_report_length) {
				/* Padding the rest with zeros. */
				memset(buf + 1 + length, 0, dev->output_report_length - length - 1);
			}
	...
	else
	{
		...
		else if (length == dev->output_report_length) {		
